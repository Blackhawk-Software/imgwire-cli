...
export default class MetricsGetDatasets extends Command {
  static override description = 'Retrieve metrics datasets'
...
`,
  ...
  public async run(): Promise<void> {
    const {flags} = await this.parse(MetricsGetDatasets)
    const result = await runSdkCommand(this, (client) =>
      client.metrics.getDatasets({
